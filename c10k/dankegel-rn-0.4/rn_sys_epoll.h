...
#ifndef rn_sys_epoll_H
#define rn_sys_epoll_H
...
/** Initialize the rn_t. */
int rn_sys_epoll_init(struct rn_sys_epoll *, int maxfds);

/** Release any resouces allocated internally by this rn_t. */
void rn_sys_epoll_shutdown(struct rn_sys_epoll *);

/**
 Sets the flags of fd for use with epoll
 On exit, the fd is ready for use with rn_sys_epoll_add
 ...
 */
int rn_sys_epoll_add(struct rn_sys_epoll *, int fd, rn_callback_fn_t pfn, void *data);

/** Remove a file descriptor. */
int rn_sys_epoll_del(struct rn_sys_epoll *, int fd);
 ...
 */
int rn_sys_epoll_waitAndDispatchEvents(struct rn_sys_epoll *, int timeout_millisec);
